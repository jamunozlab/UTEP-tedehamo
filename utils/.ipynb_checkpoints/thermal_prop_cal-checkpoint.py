...
def error_calculation(suffix, temp, defect,atom_type1,atom_type2):
    ...
    df = pd.read_csv(thermal_path + f'thermal_prop_{temp * 100 + base_params_atoms_position["initial_temperature"]}/defect{defect}/' +
    f'mech_prop_temp_{temp * 100 + base_params_atoms_position["initial_temperature"]}_def_{defect}.csv')
    
    temp = temp* 100 + base_params_atoms_position["initial_temperature"]
    ...
    compositions = np.array(df[f"{atom_type1}_composition"])
    ...
    for i in range(iterations):
        ...
        thermo_array = np.hstack([df[[f"{atom_type1}_composition"]], compositions, lattice_params, 
                                  bulk_modulus, internal_energy, df[[cdict['configurational_entropy']]]])
        
        labels = [f'{atom_type1}_composition', f'{atom_type2}_composition', cdict['lattice_parameter'], 
        ...
        # Fe_composition = get_iron_composition(suffix, temp_num,defect)
    ...
    df_debye_temp = pd.DataFrame(debye_temp_values, index = df[f"{atom_type1}_composition"], 
                                 columns = debye_columns_output_all_data)
    df_debye_temp.to_csv('Debye temp array_{}_{}.csv'.format(temp, suffix))
    
    df_vib_entropy = pd.DataFrame(vibrational_entropy_values, index = df[f"{atom_type1}_composition"], 
                                  columns = v_entr_columns_output_all_data)
    df_vib_entropy.to_csv('Vibrational entropy_{}_{}.csv'.format(temp, suffix))
    
    df_conf_entropy = pd.DataFrame(configurational_entropy_values, index = df[f"{atom_type1}_composition"], 
                                   columns = c_entr_columns_output_all_data)
    df_conf_entropy.to_csv('configurational entropy_{}_{}.csv'.format(temp, suffix))
    #configurational_entropy_0_defect%
    df_free_energy = pd.DataFrame(free_energy_values, index = df[f"{atom_type1}_composition"], 
...
def calculate_free_energy(df, suffix, temp):
    ...
    if(suffix == '_dis'):
        ...
        df[cdict['debye_temperature']] = df[cdict['lattice_parameter']].mul(1.0743e-10).mul(df[cdict['bulk_modulus']].mul(1e9)).div(df[cdict['atomic_mass']]).pow(1/2).mul(constant*0.617)
    else:
    ...
    collect_dict[cdict['free_energy']] = pd.Series(df[cdict['free_energy']].values, index=df[f'{atom_type1}_composition'].values)
...
def calculate_vibrational_entropy(df, suffix, temp):
    ...
    index = df[f'{atom_type1}_composition']
...
# def get_iron_composition(suffix, temp, defect):

#     temp_num = temp
#     suffix = suffix #'_dis'
    
#     temp = '_' + str(temp_num) + 'K'
#     df = pd.read_csv(thermal_path + f'thermal_prop_{temp_num * 100 + base_params_atoms_position["initial_temperature"]}/defect{defect}/' +
#     f'mech_prop_temp_{temp_num* 100 + base_params_atoms_position["initial_temperature"]}_def_{defect}.csv')
